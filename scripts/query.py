...
from db import close_on_or_before, connect, load_config
...
def main():
    ...
    rows = con.execute(
        """
        SELECT p.date, p.text, p.url, m.stance
    ...
    print(f"# {ticker} — {cfg['account']['display']} 提及全紀錄")
    ...
    print("\n---\n")
    for r in rows:
        print(f"## {r['date']}（{STANCE_ZH[r['stance']]}）")
        if r["url"]:
            print(r["url"])
        print(f"\n> {r['text'].strip()}\n")
